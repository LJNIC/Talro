...
-- @return nil
function Display:new(size, offset, scale, dfg, dbg, fullOrFlags, tilesetInfo, window)
   local t = {}
   setmetatable(t, self)
   self.__index = self
   local tilesetInfo = tilesetInfo or {path = 'lib/wanderlust_16x16.png', perRow = 16, charWidth = 16, charHeight = 16}
   t.tilesetChanged = false
   t.__name = 'Display'
   t.widthInChars = size and size.w or 80
   t.heightInChars = size and size.h or 24
   t.offsetX = offset and offset.x or 0
   t.offsetY = offset and offset.y or 0
   t.scale = scale or 1
   t.glyphs = {}
   t.chars = {{}}
   t.backgroundColors = {{}}
...
function Display:draw(noDraw)
   ...
   self.graphics.draw(self.canvas, self.offsetX, self.offsetY)
end
...
function Display:setTileset(tilesetInfo)
   ...
   self.glyphSprite:setFilter('nearest', 'nearest')
...
function Display:addSubDisplay(name, x1, y1, x2, y2)
	local width = x2 - x1
	local height = y2 - y1
	self.subDisplays[name] = {x1 = x1, y1 = y1, x2 = x2, y2 = y2, width = width, height = height}
end
